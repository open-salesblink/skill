...
| `email_senders` | array | ✅* | Array of senders/folders to test from (see format below). Use `sender_id` for a legacy single sender instead. |
| `sender_id` | string | ✅* | Legacy single sender UUID. Do not use together with `email_senders`. |
...
| `plainText` | boolean | | Send as plain text instead of HTML. |
| `tracking_uuid` | string | | Optional UUID for `from-outside` tests. Auto-generated if omitted. |

#### `email_senders` array format

Each item in `email_senders` must include these fields:

| Sub-field | Type | Description |
|-----------|------|-------------|
| `key` | string | UUID of the folder or sender. For folders this is the folder UUID; for senders this is the sender UUID. Same as `value`. |
| `value` | string | UUID of the folder or sender (same as `key`). The UUID used by the server. |
| `label` | string | Human-readable label, e.g. folder name or sender email address. |
| `serviceName` | string | Selection type. **MUST be `"folder"`** for folders. For individual senders use any value other than `"folder"` (commonly the provider name). |

You can mix folders and individual senders in the same array. Folders are expanded server-side into all senders they contain, and duplicates are automatically removed.
...
### Example: One-time custom content test with multiple senders
...
```json
{
  ...
  "email_senders": [
    {
      "key": "folder-uuid-here",
      "value": "folder-uuid-here",
      "label": "Sales Team Senders",
      "serviceName": "folder"
    },
    {
      "key": "sender-uuid-here",
      "value": "sender-uuid-here",
      "label": "faizan.shaikh.test@gmail.com",
      "serviceName": "gmail"
    }
  ],
  "content_type": "custom",
